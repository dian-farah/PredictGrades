...
```{r}
grades_f1 <- grades %>%
  select(StudentAbsenceDays, raisedhands, VisITedResources, AnnouncementsView, Discussion, Relation, PlaceofBirth, Topic, GradeID, Class)
```
...
importance(bag_grades)
varImpPlot(bag_grades)
```
```{r}
#grades_f2 <- grades %>%
#  select(StudentAbsenceDays, raisedhands, VisITedResources, AnnouncementsView, Discussion, #Relation, PlaceofBirth, Topic, GradeID, gender, NationalITy, StageID, SectionID, ParentAnsweringSurvey, ParentschoolSatisfaction, Class)
...
bag_grades1 <- randomForest(Class~.-Semester, data=train, importance=TRUE)
bag_grades1
